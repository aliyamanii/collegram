...
interface AddPostModalProps {
  isOpen: boolean;
  closeModal: () => void;
  onSubmit: (bioInput: { bio: string }) => void;
  onCancel: () => void;
}
...
const EditProfileModal: React.FC<AddPostModalProps> = ({
  ...
  const handleInputChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      ...
      setBioInput((prevData) => ({ ...prevData, [name]: value }));
    },
    []
  );
  ...
  return (
    <div className={`fixed  inset-0 z-10 ${isOpen ? "block" : "hidden"}`}>
      <div
        ...
        onClick={closeModal}
      ></div>
      <div className="fixed inset-0 flex items-center justify-center p-4 text-center hover:cursor-default">
        <div className="w-[375px] p-12 flex flex-col gap-7 align-middle transform bg-[#F3F0EE] rounded-[24px] shadow-xl transition-all">
